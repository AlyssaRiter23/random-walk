...
import legacy
import dnnlib
from time import perf_counter # measures the execution time of the random walk
...
# NEW function to load the image and project it to latent space
def project_image_to_latent_space(G, image_path, device):
    image = Image.open(image_path).convert('RGB')
    image = image.resize((1024,1024))
    image_tensor = torch.tensor(np.array(image).transpose(2,0,1), dtype=torch.float32, device=device).unsqueeze(0) / 127.5 - 1.0

    with torch.no_grad():
        latent_vector = G.mapping(image_tensor)

    return latent_vector

# function for clustering the latent vectors
def cluster_latents(latent_vector):
    # perfrom PCA to reduce dimensionality for clustering
    pca = PCA(n_components=2)
    reduced_latents = pca.fit_transform(latent_vectors)

    # apply the KMeans clustering
    kmeans = KMeans(n_clusters=3, random_state=42)
    kmeans.fit(reduced_latents)
...
@click.option('--image', help='Input image to project', required=True)
def run_random_walk(
    ...
    print("projecting input image into latent space...")
    latent_vector = project_image_to_latent_space(G, image, device)
    print(f"Image Latent Vector: {latent_vector.shape}")
    ...
    cluster_labels, cluster_centers = cluster_latents(w_out.cpu().numpy())
