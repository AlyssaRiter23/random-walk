...
def synthesize_write_batch(G,batch,base,outdir):
    batch = torch.stack(batch).squeeze().to(torch.float32)   # whee!
    with torch.no_grad(): img_batch = G.synthesis(batch, noise_mode='const').float()
...
def run_random_walk(
    ...
    # added to try and get it to run
